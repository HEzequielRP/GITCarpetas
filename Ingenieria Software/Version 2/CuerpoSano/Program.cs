...
namespace CuerpoSano
{
    public class Program
    {
            ...
            while (!salir)
            {
                ...
                switch (opcion)
                {
                    case 1:
                        ...
                        break;
                    case 2:
                        MenuMiembros(servicioMiembro, servicioMembresia);
                        break;
                    case 3:
                        MenuEntrenadores(servicioEntrenador);
                        break;
                    case 4:
                        MenuClases(servicioClase, servicioEntrenador);
                        break;
                    case 5:
        ...
        private static void MenuMiembros(GM_MiembroService servicio, GP_MembresiaService servicioMembresia)
        {
            bool volver = false;
            while (!volver)
            {
        ...
        private static void MenuClases(GC_ClaseService servicio, GE_EntrenadorService servicioEntrenador)
        {
            bool volver = false;
            while (!volver)
            {
        ...
        private static void ListarMiembros(GM_MiembroService servicio, bool pausar = true)
        {
            ...
            else
            {
                ...
                {
                    Console.WriteLine($"Id: {m.Id}, Nombre: {m.Nombre} {m.Apellido}, Documento: {m.TipoDocumento} {m.NumeroDocumento}, Codigo: {m.CodigoAlumno}, MembresiaId: {m.MembresiaId}");
                }
            }
            if (pausar) Pausar();
        }
        ...
        private static void ListarEntrenadores(GE_EntrenadorService servicio, bool pausar = true)
        {
            ...
            else
            {
                ...
                {
                    Console.WriteLine($"Id: {p.Id}, Nombre: {p.Nombre} {p.Apellido}, Codigo: {p.CodigoProfesor}, Especialidad: {p.Especialidad}");
                }
            }
            if (pausar) Pausar();
        }
        ...
        private static void ListarClases(GC_ClaseService servicio, bool pausar = true)
        {
            ...
            var clases = servicio.ObtenerTodos();
            if (clases.Count == 0)
            {
            ...
            else
            {
                ...
                {
                    Console.WriteLine($"Id: {c.Id}, Nombre: {c.Nombre}, Horario: {c.Horario}, Cupo: {c.CupoMaximo}, ProfesorId: {c.ProfesorId}");
                }
            }
            if (pausar) Pausar();
        }
